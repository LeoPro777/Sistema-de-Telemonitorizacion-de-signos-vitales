...
export const DoctorDashboard: React.FC<DoctorDashboardProps> = ({ kpis }) => {
  ...
  // Estilo dinámico de telemetría según el status
    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-2 gap-3 md:gap-6">
        ...
        <div className="bg-[#1E2640] p-4 md:p-6 rounded-2xl md:rounded-3xl border border-[#1E2640] hover:border-[#D4AF37]/35 transition-all flex flex-col justify-between h-32 md:h-40 group shadow-lg">
          <div className="flex justify-between items-start">
            ...
            <span className="text-[8px] md:text-[10px] text-slate-500 font-bold uppercase tracking-widest font-mono">ASIGNACIÓN</span>
          </div>
        ...
        <div className={`p-4 md:p-6 rounded-2xl md:rounded-3xl border transition-all flex flex-col justify-between h-32 md:h-40 group shadow-lg ${
          ...
          <div className="flex justify-between items-start">
            <div className={`h-9 w-9 md:h-12 md:w-12 rounded-xl md:rounded-2xl flex items-center justify-center border ${
              (kpis.critical_patients_count || 0) > 0 
              ...
              <ShieldAlert className={`h-4 w-4 md:h-5 md:w-5 ${(kpis.critical_patients_count || 0) > 0 ? 'animate-bounce' : ''}`} />
            </div>
            <span className="text-[8px] md:text-[10px] text-slate-500 font-bold uppercase tracking-widest font-mono">ESTADO REAL</span>
          </div>
          <div className="my-1 md:my-2">
            <span className={`text-2xl md:text-4xl font-extrabold tracking-tight block ${
      ...
      {criticalPatients.length === 0 ? (
        <div className="bg-[#1E2640] p-12 rounded-3xl border border-[#1E2640] shadow-xl flex flex-col items-center justify-center text-center space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-400 mt-2 max-w-md">
              Actualmente no se registran pacientes con telemetría fuera de los umbrales clínicos de seguridad en su guardia.
            </p>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-6">
          ...
          <div className="bg-[#1E2640] p-4 md:p-6 rounded-2xl md:rounded-3xl border border-[#1E2640] lg:col-span-6 flex flex-col shadow-xl">
            <div className="mb-4 md:mb-6">
              <span className="text-[10px] text-[#FF1744] tracking-[0.2em] font-bold uppercase block mb-1">TRIAGE REDUCIDO</span>
              <h3 className="text-base md:text-lg font-bold text-white">Monitoreo Fisiológico de Pacientes Críticos</h3>
              <p className="text-[11px] md:text-xs text-slate-400 mt-1">
                Visualización de última telemetría biométrica de pacientes en estado clínico crítico.
              </p>
            </div>
 
            <div className="space-y-2.5 overflow-y-auto max-h-[30rem] pr-2 custom-scrollbar">
              {criticalPatients.map((patient: any) => {
                ...
                return (
                  <div
                    ...
                    className={`p-3 md:p-4 rounded-xl md:rounded-2xl border cursor-pointer flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 transition-all duration-200 hover:scale-[1.01] ${
                      isSelected 
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2 mt-1 text-[11px] md:text-xs text-slate-400">
                    ...
                    <div className="grid grid-cols-3 gap-2 md:gap-3 w-full sm:w-auto">
                      
                      {/* Pulso */}
                      <div className={`p-1.5 md:p-2 rounded-lg md:rounded-xl border flex flex-col items-center min-w-[4.8rem] sm:min-w-[5.5rem] ${getStatusBg(telemetry.heart_rate?.status)}`}>
                        <div className="flex items-center space-x-1 mb-0.5 text-slate-400">
                          <Heart className="h-2.5 w-2.5 md:h-3 md:w-3 text-red-500" />
                          <span className="text-[8px] md:text-[9px] font-bold">Pulso</span>
                        </div>
                        <span className={`text-[11px] md:text-xs font-bold tracking-tight ${getStatusColor(telemetry.heart_rate?.status)}`}>
                          {telemetry.heart_rate?.value ? `${Math.round(telemetry.heart_rate.value)}` : '--'} <span className="text-[7px] md:text-[8px] text-slate-500 font-normal">bpm</span>
                        </span>
                      </div>
 
                      {/* Oxigenación */}
                      <div className={`p-1.5 md:p-2 rounded-lg md:rounded-xl border flex flex-col items-center min-w-[4.8rem] sm:min-w-[5.5rem] ${getStatusBg(telemetry.spo2?.status)}`}>
                        <div className="flex items-center space-x-1 mb-0.5 text-slate-400">
                          <Activity className="h-2.5 w-2.5 md:h-3 md:w-3 text-cyan-400" />
                          <span className="text-[8px] md:text-[9px] font-bold">SpO2</span>
                        </div>
                        <span className={`text-[11px] md:text-xs font-bold tracking-tight ${getStatusColor(telemetry.spo2?.status)}`}>
                          {telemetry.spo2?.value ? `${Math.round(telemetry.spo2.value)}` : '--'} <span className="text-[7px] md:text-[8px] text-slate-500 font-normal">%</span>
                        </span>
                      </div>
 
                      {/* Temperatura */}
                      <div className={`p-1.5 md:p-2 rounded-lg md:rounded-xl border flex flex-col items-center min-w-[4.8rem] sm:min-w-[5.5rem] ${getStatusBg(telemetry.temperature?.status)}`}>
                        <div className="flex items-center space-x-1 mb-0.5 text-slate-400">
                          <Thermometer className="h-2.5 w-2.5 md:h-3 md:w-3 text-amber-500" />
                          <span className="text-[8px] md:text-[9px] font-bold">Temp</span>
                        </div>
                        <span className={`text-[11px] md:text-xs font-bold tracking-tight ${getStatusColor(telemetry.temperature?.status)}`}>
                          {telemetry.temperature?.value ? `${parseFloat(telemetry.temperature.value).toFixed(1)}` : '--'} <span className="text-[7px] md:text-[8px] text-slate-500 font-normal">°C</span>
                        </span>
                      </div>
          ...
          <div className="lg:col-span-4 flex flex-col">
            {selectedPatient ? (
              <div className="bg-[#1E2640] p-4 md:p-6 rounded-2xl md:rounded-3xl border border-[#D4AF37]/20 shadow-xl flex-grow flex flex-col justify-between space-y-4 md:space-y-6">
                <div>
